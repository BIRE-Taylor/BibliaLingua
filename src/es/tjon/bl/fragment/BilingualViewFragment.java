...
public class BilingualViewFragment extends Fragment
{
	...
	public void setDisplayMode(boolean primary, boolean secondary, boolean related, String reference)
	{
		...
		if (mState.mDisplayRelated) AsyncTask.execute(new Runnable(){String mRef;public Runnable setup(String reference)
												   ...
												   {getActivity().runOnUiThread(new Runnable(){String mRef;public Runnable setup(String reference)
																	  {mRef = reference;return this;}@Override
																	  public void run()
																	  { mContentPrimary.loadUrl("javascript:ldssa.main.getOffsetsForRcaItems();"); if (mRef == null) mContentRelated.scrollTo(mContentPrimary.findFirstRCAItem()); else mContentRelated.scrollTo(mRef);}}.setup(mRef));}}.setup(reference));
	}
